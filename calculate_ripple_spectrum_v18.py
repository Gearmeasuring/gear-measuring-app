"""
波纹度频谱分析 V18 - 基于ep和el的角度映射

关键理解：
- ep = 评价长度/基节 = 1.454，每个齿形数据对应1.454个基节的展开长度
- el = 评价长度/基节 = 2.766，每个齿向数据对应2.766个基节的展开长度
- 每个基节对应 360°/ZE 的角度

角度映射：
- 齿形：每个齿的数据对应 ep × (360°/ZE) 的角度范围
- 齿向：每个齿的数据对应 el × (360°/ZE) 的角度范围
"""
import os
import sys
import math
...
def build_closed_curve_by_ep_el(all_tooth_data, teeth_count, ep_or_el, data_type='profile'):
    """
    基于ep/el构建闭合曲线
    ...
    每个齿的数据对应 ep_or_el × (360°/ZE) 的角度范围
    """
...
def analyze_ripple(mka_file):
    """分析波纹度"""
    print("="*70)
    print("波纹度频谱分析 V18 (基于ep/el的角度映射)")
    print("="*70)
    print("\n关键理解:")
    print("1. ep = 评价长度/基节 = 1.454，每个齿形数据对应1.454个基节")
    print("2. el = 评价长度/基节 = 2.766，每个齿向数据对应2.766个基节")
    print("3. 每个基节对应 360°/ZE 的角度")
    print("4. 齿形: 每齿数据对应 ep × (360°/ZE) ≈ 6.02° 的角度范围")
    print("5. 齿向: 每齿数据对应 el × (360°/ZE) ≈ 11.45° 的角度范围")
    ...
    print(f"\n【评价参数】")
    print(f"  ep = {ep:.4f} (评价长度/基节)")
    print(f"  el = {el:.4f} (评价长度/基节)")
    print(f"  齿形每齿角度范围 = ep × 360°/ZE = {ep * 360 / teeth_count:.4f}°")
    print(f"  齿向每齿角度范围 = el × 360°/ZE = {el * 360 / teeth_count:.4f}°")
    ...
    for side, data_type, name, data_source, ep_or_el in directions:
        ...
        angles, values = build_closed_curve_by_ep_el(
