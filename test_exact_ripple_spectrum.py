#!/usr/bin/env python3
"""
精确测试脚本：使用与用户提供图表完全一致的测试数据测试Klingelnberg波纹度频谱报告生成器
"""
...
class BasicInfo:
    ...
    drawing_no: str = "84-T3.2.47.02.76-G-WAV"
    customer: str = "Test Customer"
    order_no: str = "263751-018-WAV"
    type: str = "gear"
    program: str = "263751-018-WAV"
    date: str = "14.02.25"
...
# 生成与用户提供图表完全一致的测试数据
def generate_exact_test_data(teeth_count=87, num_teeth=10, num_points=200):
    """生成与用户提供图表完全一致的测试数据"""
    profile_data = {}
    flank_data = {}
    ...
    for tooth_id in range(num_teeth):
        ...
        # 添加2ZE阶次（174）
        signal += 0.50 * np.sin(2 * teeth_count * x + 0.2 * tooth_id)
        
        # 添加3ZE阶次（261）
        signal += 0.43 * np.sin(3 * teeth_count * x + 0.3 * tooth_id)
        
        # 添加4ZE阶次（348）
        signal += 0.25 * np.sin(4 * teeth_count * x + 0.4 * tooth_id)
...
def test_exact_ripple_spectrum():
    """测试与用户提供图表完全一致的波纹度频谱报告生成器"""
    print("=== 测试与用户提供图表完全一致的 Klingelnberg 波纹度频谱报告生成器 ===")
    
    # 创建基本信息
    basic_info = BasicInfo()
    
    # 生成与用户提供图表完全一致的测试数据
    profile_data, flank_data = generate_exact_test_data()
    ...
    # 更新显示设置，确保最小幅值阈值为0.02
    settings.update_display_settings(
        min_amplitude_um=0.02
    )
    ...
    try:
        ...
        with PdfPages('exact_test_ripple_spectrum.pdf') as pdf:
            report.create_page(pdf, measurement_data)
        print("Successfully generated exact PDF report")
