...
关键理解：
- ep = 1.454 → 每个齿形数据覆盖1.454个基节的展开长度
- el = 2.766 → 每个齿向数据覆盖2.766个基节的展开长度
- 每个基节对应 360°/ZE 的角度
- 齿形：每齿数据覆盖 1.454 × 4.14° ≈ 6.02°，但齿中心间隔4.14°
- 齿向：每齿数据覆盖 2.766 × 4.14° ≈ 11.45°，但齿中心间隔4.14°
- 数据会有重叠！
"""
import os
import sys
...
def build_closed_curve_overlapping(all_tooth_data, teeth_count, ep_or_el, data_type='profile'):
    """
    构建闭合曲线 - 重叠排列方式
    
    每个齿的数据覆盖 ep_or_el 个基节的角度范围
    齿中心间隔 360°/ZE
    数据会有重叠
    """
    ...
    print(f"    每齿中心间隔: {angle_per_tooth:.4f}°")
    print(f"    每齿数据角度范围: {angle_per_data:.4f}° (ep/el × 360°/ZE)")
    print(f"    重叠角度: {angle_per_data - angle_per_tooth:.4f}°")
    ...
    for tooth_idx, tooth_data in enumerate(all_tooth_data):
        ...
        angle_range = np.linspace(-angle_per_data/2, angle_per_data/2, n_points)
        alpha = angle_range + tooth_center
    ...
    n_bins = int((angle_max - angle_min) / 0.01) + 1
    bin_edges = np.linspace(angle_min, angle_max, n_bins)
    
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    bin_values = []
    
    for i in range(len(bin_edges) - 1):
        mask = (all_angles >= bin_edges[i]) & (all_angles < bin_edges[i+1])
        if np.any(mask):
            bin_values.append(np.mean(all_values[mask]))
    
    if len(bin_values) == 0:
        return None, None
    
    return bin_centers[:len(bin_values)], np.array(bin_values)


def analyze_ripple(mka_file):
    """分析波纹度"""
    print("="*70)
    print("波纹度频谱分析 V19 (重叠排列方式)")
    print("="*70)
    print("\n关键理解:")
    print("1. ep = 1.454 → 每个齿形数据覆盖1.454个基节")
    print("2. el = 2.766 → 每个齿向数据覆盖2.766个基节")
    print("3. 齿中心间隔 = 360°/ZE = 4.14°")
    print("4. 齿形数据覆盖6.02°，有1.88°重叠")
    print("5. 齿向数据覆盖11.45°，有7.31°重叠")
    ...
    for side, data_type, name, data_source, ep_or_el in directions:
        ...
        angles, values = build_closed_curve_overlapping(
