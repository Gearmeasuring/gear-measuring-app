"""
根据Klingelnberg论文重新实现波纹度分析算法
关键特点:
1. 迭代正弦波分解 (Iterative Sine Wave Decomposition)
2. 补偿正弦波函数计算
3. 选择频率范围分析
4. 10次迭代提取主要频率
"""
...
class KlingelnbergRippleAnalyzer:
    """
    Klingelnberg波纹度分析器
    基于论文 "Analysis of ripple on noisy gears" (AGMA 2012)
    """
    ...
    def build_merged_curve(self, side_data, data_type, side, eval_start, eval_end, meas_start, meas_end):
        ...
        for tooth_id in sorted_teeth:
            ...
            # 每齿预处理 - 去除鼓形和斜率 (根据论文第5节)
            corrected_values = self.preprocess_tooth(eval_values)
            ...
            if data_type == 'profile':
                ...
                if side == 'left':
                    angles = tau - xi_angles  # 左齿形
                else:
                    angles = tau + xi_angles  # 右齿形
            else:
                # Helix: φ = -Δφ + τ (齿向)
                ...
                if side == 'left':
                    angles = tau - delta_phi  # 左齿向
                else:
                    angles = tau + delta_phi  # 右齿向
    ...
    def preprocess_tooth(self, values, poly_order=2):
        """
        单齿预处理 - 去除鼓形和斜率
        根据论文第5节，必须去除这些系统误差
        """
        ...
        coeffs = np.polyfit(x_norm, values, poly_order, w=weights)
    ...
    def iterative_sine_decomposition(self, angles, values, n_cycles=10, freq_range=None):
        """
        迭代正弦波分解
        根据论文第3节: "After 10 cycles, this ultimately produces a frequency spectrum of the 10 maximum amplitudes"
        ...
        n_cycles : int
            迭代次数 (默认10次)
        freq_range : tuple
            频率范围 (min_order, max_order)，None表示全范围
        """
        ...
        if freq_range is None:
            min_order = 1
            max_order = 5 * self.teeth_count  # 最高5倍齿数
        else:
...
def main():
    ...
    print("=" * 90)
    print("Klingelnberg波纹度分析 (基于AGMA 2012论文)")
    print("=" * 90)
    ...
    analyzer = KlingelnbergRippleAnalyzer(
    ...
    test_configs = [
        ...
        (15, None, "15次迭代, 全频段"),
        (20, None, "20次迭代, 全频段"),
        (10, (80, 440), "10次迭代, 80-440阶次"),
        (15, (80, 440), "15次迭代, 80-440阶次"),
    ]
    ...
    for n_cycles, freq_range, desc in test_configs:
        print(f"\n测试配置: {desc}")
        ...
        for direction in directions:
            components, angles, values = analyzer.analyze_direction(
                ...
                n_cycles=n_cycles, freq_range=freq_range
            )
    ...
    plt.tight_layout()
    output_path = os.path.join(current_dir, "klingelnberg_analysis_result.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"\n分析结果图已保存: {output_path}")
    
    # 保存详细结果到CSV
    csv_path = os.path.join(current_dir, "klingelnberg_analysis_results.csv")
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write("配置,方向,阶次,我们的振幅,Klingelnberg振幅,误差(%),迭代次数\n")
        ...
        for result in all_results:
            for direction in directions:
                ...
                for order in sorted(ref.keys()):
                    ...
                    f.write(f"{result['config']},{direction},{order},{our_amp:.4f},{ref_amp:.4f},{error:.1f},{result['n_cycles']}\n")
