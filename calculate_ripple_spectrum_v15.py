"""
波纹度频谱分析 V15 - 使用正确的RC滤波器参数

关键改进：
1. 报告显示 "Low-pass filter RC 100000:1"
2. 完整360°闭合曲线（缺失齿插值填充）
3. 迭代提取所有阶次
4. 评价时只考虑阶次≥ZE的成分
"""
import os
...
def apply_rc_low_pass_filter(data, fs=10000.0, ratio=100000.0):
    """
    Klingelnberg RC 低通滤波器
    
    RC 100000:1 表示截止频率 = fs / 100000
    """
...
def analyze_ripple(mka_file):
    ...
    for side, data_type, name, data_source, eval_start, eval_end in directions:
        ...
        filtered_values = apply_rc_low_pass_filter(values, fs=10000.0, ratio=100000.0)
