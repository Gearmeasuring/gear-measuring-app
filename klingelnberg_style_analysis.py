...
# -*- coding: utf-8 -*-
"""
基于Klingelnberg文档的波纹度分析

关键发现：
1. 低阶评价：保留ZE、2ZE、3ZE等阶次，消除更低的频率
2. 高阶评价：消除鼓形和齿距偏差，显示更高阶的波纹度
3. 拟合正弦是优先选用的最精确的计算方法
"""
...
def remove_low_frequency_trend(phi, dev, ZE, cutoff_order=0.5):
    ...
    # 使用多项式拟合来近似低频趋势
    # 多项式阶数取决于截止频率
    poly_order = int(np.ceil(cutoff_freq * 10))
    poly_order = max(1, min(poly_order, 5))
...
def analyze_with_klingelnberg_method(mka_file, sample_name, klingelnberg_ref, evaluation_type='low'):
    """
    使用Klingelnberg方法分析
    
    Args:
        evaluation_type: 'low' (低阶), 'high' (高阶), 'all' (所有阶)
    """
    print(f"\n{'='*80}")
    print(f"Klingelnberg方法分析: {sample_name}")
    print(f"评价类型: {evaluation_type} ({'低阶' if evaluation_type=='low' else '高阶' if evaluation_type=='high' else '所有阶'})")
    print(f"{'='*80}\n")
    ...
    if evaluation_type == 'low':
        cutoff_order = 0.5  # 消除0.5*ZE以下的频率
    elif evaluation_type == 'high':
        cutoff_order = 1.0  # 消除ZE以下的频率
    else:
        cutoff_order = 0.3  # 轻微预处理
...
def main():
    print("="*80)
    print("基于Klingelnberg文档的波纹度分析")
    print("="*80)
    
    # 测试三种评价类型
    for eval_type, eval_name in [('low', '低阶'), ('high', '高阶'), ('all', '所有阶')]:
        print(f"\n\n{'#'*80}")
        print(f"# 评价类型: {eval_name}")
        print(f"{'#'*80}")
