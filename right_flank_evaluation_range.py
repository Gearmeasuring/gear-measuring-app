#!/usr/bin/env python3
"""
右齿向去鼓形、倾斜后的评价范围内的曲线，按照旋转角的公式合并，生成图表查看每一段曲线是否正常
"""
...
def get_evaluation_range_indices(data_length, eval_ranges):
    ...
    根据评价范围计算数据索引范围
    """
    # 尝试使用 'lead' 键，如果不存在则使用 'profile' 键
    if 'lead' in eval_ranges:
        lead_start = eval_ranges['lead']['start']
        lead_end = eval_ranges['lead']['end']
        lead_start_mess = eval_ranges['lead']['start_mess']
        lead_end_mess = eval_ranges['lead']['end_mess']
    else:
        # 回退到使用 profile 评价范围
        lead_start = eval_ranges['profile']['start']
        lead_end = eval_ranges['profile']['end']
        lead_start_mess = eval_ranges['profile']['start_mess']
        lead_end_mess = eval_ranges['profile']['end_mess']
    
    if lead_start > 0 and lead_end > lead_start and lead_start_mess >= 0 and lead_end_mess > lead_start_mess:
        # 计算评价范围在测量范围内的比例
        total_mess_range = lead_end_mess - lead_start_mess
        eval_start_offset = lead_start - lead_start_mess
        eval_end_offset = lead_end - lead_start_mess
...
def process_and_merge_right_flank_evaluation_range():
    """
    处理右齿向评价范围内的数据，去鼓形和倾斜，然后按照旋转角公式合并
    """
    print("=== 处理右齿向评价范围内的数据，去鼓形和倾斜，按照旋转角公式合并 ===")
    ...
    # 检查是否有 lead 评价范围，如果没有则使用 profile
    if 'lead' in eval_ranges:
        print(f"齿向评价范围: {eval_ranges['lead']['start']:.3f} - {eval_ranges['lead']['end']:.3f} mm")
        print(f"齿向测量范围: {eval_ranges['lead']['start_mess']:.3f} - {eval_ranges['lead']['end_mess']:.3f} mm")
    else:
        print(f"使用齿形评价范围作为齿向评价范围")
        print(f"评价范围: {eval_ranges['profile']['start']:.3f} - {eval_ranges['profile']['end']:.3f} mm")
        print(f"测量范围: {eval_ranges['profile']['start_mess']:.3f} - {eval_ranges['profile']['end_mess']:.3f} mm")
    
    # 获取右齿向数据
    right_flank_data = parser.flank_data['right']
    
    print(f"右齿向数量: {len(right_flank_data)}")
    ...
    sorted_tooth_ids = sorted(right_flank_data.keys(), key=extract_number)
    ...
    for i, tooth_id in enumerate(sorted_tooth_ids):
        # 获取原始数据
        raw_data = right_flank_data[tooth_id]
...
def plot_merged_right_flank_evaluation_range():
    """
    生成合并后的右齿向评价范围内的曲线图表
    """
    # 处理和合并数据
    (
        ...
        rotation_angles_per_tooth,
        evaluation_ranges_per_tooth
    ) = process_and_merge_right_flank_evaluation_range()
    ...
    ax1.plot(merged_rotation_angles, merged_processed_data, color='green', linewidth=1.0, alpha=0.8)
    ax1.set_title(f'Merged Right Flank Curve (Evaluation Range) (z = {len(sorted_tooth_ids)})', fontsize=12, fontweight='bold')
    ...
    ax2.set_title('First 5 Teeth Flanks (Evaluation Range, After Correction)', fontsize=12, fontweight='bold')
    ...
    # 保存图表
    output_file = 'right_flank_evaluation_range_merged.png'
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"\n=== 图表生成完成 ===")
    print(f"合并后的右齿向评价范围曲线图表已保存为: {output_file}")
...
def plot_individual_teeth_evaluation_range():
    ...
    for i, tooth_id in enumerate(sorted_tooth_ids[:10]):
        ...
        # 找到该齿号在排序后的列表中的索引
        tooth_index = None
        for j, tid in enumerate(sorted_tooth_ids):
            if tid == tooth_id:
                tooth_index = j
                break
        ...
        if tooth_index is not None:
            ...
            # 绘制处理后的数据
            ax.plot(rotation_angles, processed_data, color='green', linewidth=1.2, alpha=0.9, label='Processed Data')
            
            # 添加标题和标签
            ax.set_title(f'Tooth {tooth_id} - Right Flank Evaluation Range (Indices: {start_idx}-{end_idx})', fontsize=12, fontweight='bold')
            ...
            # 保存图表
            output_file = f'tooth_{tooth_id}_right_flank_evaluation_correction.png'
            plt.savefig(output_file, dpi=150, bbox_inches='tight')
            print(f"生成齿 {tooth_id} 的右齿向评价范围图表: {output_file}")
    ...
    for tooth_id in specified_teeth:
        if tooth_id in right_flank_data:
            # 获取原始数据
            raw_data = right_flank_data[tooth_id]
            ...
            print(f"齿 {tooth_id} 不在右齿向数据中")


if __name__ == '__main__':
    plot_merged_right_flank_evaluation_range()
