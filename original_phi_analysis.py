...
def fit_sine_at_order_with_freq(phi, signal, order, ZE):
    ...
    关键：频率 = order / (phi_range)
    其中phi_range是phi的总范围（弧度）
    """
    phi_range = phi.max() - phi.min()
    
    # 频率 = order / phi_range
    # 这样order个周期正好覆盖phi_range
...
def analyze_with_original_phi(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    print(f"\n{'='*80}")
    print(f"使用原始角度的波纹度分析: {sample_name}")
...
def main():
    ...
    print("="*80)
    print("\n关键算法：")
    print("1. 使用原始phi值进行拟合")
    print("2. 频率 = order / phi_range")
    print("3. 预处理消除低频趋势")
    ...
    for poly_order in [2, 3]:
        ...
        # 分析样本1
        error1 = analyze_with_original_phi(
            "263751-018-WAV.mka",
        ...
        # 分析样本2
        error2 = analyze_with_original_phi(
            "004-xiaoxiao1.mka",
