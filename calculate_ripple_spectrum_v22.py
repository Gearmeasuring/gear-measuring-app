"""
波纹度频谱分析 V22 - 按ep/el排列，齿中心间隔360°/ZE

关键理解：
- ep = 1.454 → 每个齿形数据覆盖1.454个基节的角度范围
- el = 2.766 → 每个齿向数据覆盖2.766个基节的角度范围
- 齿中心间隔仍然是 360°/ZE
- 数据会有重叠，对重叠区域进行平均
- 形成完整的360°闭合曲线
"""
...
def build_closed_curve_ep_el_center_spacing(all_tooth_data, teeth_count, ep_or_el, data_type='profile'):
    """
    按ep/el排列，齿中心间隔360°/ZE
    ...
    print(f"    齿中心间隔: {angle_per_tooth:.4f}° (360°/ZE)")
    ...
    for tooth_idx, tooth_data in enumerate(all_tooth_data):
        ...
        angle_range = np.linspace(tooth_center - angle_per_data/2, tooth_center + angle_per_data/2, n_points)
    ...
    all_angles = all_angles + (360 if np.min(all_angles) < 0 else 0)
    
    all_angles = np.mod(all_angles, 360)
...
def analyze_ripple(mka_file):
    """分析波纹度"""
    print("="*70)
    print("波纹度频谱分析 V22 (按ep/el排列，齿中心间隔360°/ZE)")
    print("="*70)
    print("\n关键理解:")
    print("1. ep = 1.454 → 每个齿形数据覆盖1.454个基节")
    print("2. el = 2.766 → 每个齿向数据覆盖2.766个基节")
    print("3. 齿中心间隔 = 360°/ZE")
    print("4. 数据会有重叠，对重叠区域进行平均")
    print("5. 形成完整的360°闭合曲线")
    ...
    for side, data_type, name, data_source, ep_or_el in directions:
        ...
        angles, values = build_closed_curve_ep_el_center_spacing(
