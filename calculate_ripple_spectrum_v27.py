...
根据Klingelnberg文档优化：
1. 齿廓（Profile）：
   - 左侧齿形：从齿根（小直径）开始，到齿顶（大直径）
   - 右侧齿形：从齿根（小直径）开始，到齿顶（大直径）
2. 齿向（Helix）：
   - 右旋齿轮：从上方开始
   - 左旋齿轮：从下方开始
...
2. Helix: 轴向位置 z → 旋转角 α₂ = 2×Δz×tan(β₀)/D₀
3. 根据左右侧调整方向
4. 合并所有齿形成360°闭合曲线
5. 齿形数据缩放因子：ep × 4.5
6. 迭代提取：每次提取幅值最大的阶次
...
def build_closed_curve_with_direction(all_tooth_data, teeth_count, base_diameter, data_type='profile', 
                                       eval_start=0, eval_end=0, helix_angle=0, pitch_diameter=0,
                                       side='left', is_right_hand=True):
    """
    构建完整闭合曲线 - 考虑方向
    ...
    print(f"    原始有效齿数据: {len(tooth_data_dict)} 齿")
    ...
    if missing_teeth:
        print(f"    缺失齿号: {[t+1 for t in missing_teeth]}")
    ...
    for tooth_idx in range(teeth_count):
        ...
        if data_type == 'profile':
            ...
            # 齿向：根据螺旋方向和左右侧调整
            # 右旋齿轮：左侧从下方开始，右侧从上方开始
            # 左旋齿轮：左侧从上方开始，右侧从下方开始
            if is_right_hand:
                # 右旋
                if side == 'left':
                    # 左齿向：从下方（小z）开始
...
def analyze_ripple(mka_file):
    """分析波纹度"""
    print("="*70)
    print("波纹度频谱分析 V27 (优化方向处理)")
    print("="*70)
    print("\n根据Klingelnberg文档优化方向：")
    print("1. 齿廓：从齿根（小直径）开始，到齿顶（大直径）")
    print("2. 齿向：右旋从上方开始，左旋从下方开始")
    ...
    print(f"  螺旋角 β = {helix_angle}° ({'右旋' if is_right_hand else '左旋'})")
    print(f"  节圆直径 d = {pitch_diameter:.4f} mm")
    print(f"  基圆直径 db = {base_diameter:.4f} mm")
    print(f"\n【评价参数】")
    print(f"  ep = {ep:.4f}")
    print(f"  el = {el:.4f}")
    print(f"  齿形缩放因子 = ep × 4.5 = {ep * 4.5:.4f}")
    ...
    directions = [
        ('left', 'profile', '左齿形', profile_data, d1, d2, ep * 4.5),
        ('right', 'profile', '右齿形', profile_data, d1, d2, ep * 4.5),
    ...
    for side, data_type, name, data_source, eval_start, eval_end, scale_factor in directions:
        ...
        angles, values = build_closed_curve_with_direction(
        ...
        print(f"  缩放后数据范围: {np.min(scaled_values):.4f} ~ {np.max(scaled_values):.4f} μm")
        
        spectrum = iterative_sine_fit_all_orders(scaled_values, teeth_count, max_components=10, max_order=500)
