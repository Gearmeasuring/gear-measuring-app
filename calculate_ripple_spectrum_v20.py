"""
波纹度频谱分析 V20 - 按ep/el排列完整圆周

关键理解：
- ep = 1.454 → 齿形评价长度是1.454个基节
- el = 2.766 → 齿向评价长度是2.766个基节
- 每个基节对应 360°/ZE 的角度
- 所有齿的数据按 ep/el × 360°/ZE 的总角度范围排列
- 形成完整的圆周曲线
"""
import os
import sys
...
def build_closed_curve_by_ep_el_v2(all_tooth_data, teeth_count, ep_or_el, data_type='profile'):
    """
    按ep/el排列完整圆周
    
    每个齿的数据对应 ep_or_el 个基节的角度范围
    所有齿的数据按此范围排列，形成完整的圆周曲线
    """
...
def analyze_ripple(mka_file):
    ...
    for side, data_type, name, data_source, ep_or_el in directions:
        ...
        angles, values = build_closed_curve_by_ep_el_v2(
