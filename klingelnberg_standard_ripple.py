...
class KlingelnbergRippleAnalyzer:
    ...
    def iterative_sine_decomposition(self, angles, values, max_components=10):
        ...
        核心算法（Klingelnberg标准方法）：
        1. 在1到5×ZE范围内搜索所有阶次
        2. 通过最小二乘法分解阶次最大的正弦波
        3. 从原始信号中移除已提取的最大阶次正弦波
        4. 对剩余信号重复上述过程
        5. 直到提取出10个较大的阶次
        """
        ...
        for iteration in range(max_iterations):
            ...
            for order in range(1, max_order + 1):
                if order not in spectrum_results:
...
def analyze_sample_klingelnberg(mka_file, sample_name, klingelnberg_ref):
    """使用Klingelnberg标准算法分析样本"""
    print(f"\n{'='*90}")
    print(f"Klingelnberg Standard Analysis: {sample_name}")
    print(f"{'='*90}")
...
def main():
    ...
    result1 = analyze_sample_klingelnberg(sample1_file, "Sample1 (ZE=87)", KLINGELNBERG_SAMPLE1)
    result2 = analyze_sample_klingelnberg(sample2_file, "Sample2 (ZE=26)", KLINGELNBERG_SAMPLE2)
    ...
    print(f"""
Klingelnberg Standard Algorithm:
  1. Angle Synthesis: phi = -xi - delta_phi + tau
     - Profile: phi = -xi + tau (delta_phi = 0)
     - Helix: phi = -delta_phi + tau (xi = 0)
  2. Preprocessing: Remove crowning and slope (3rd order polynomial)
  3. Iterative Sine Decomposition:
     - Only use ZE multiples as candidate orders
     - Extract 10 largest components
