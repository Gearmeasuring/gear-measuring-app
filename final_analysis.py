...
关键发现：
1. 迭代分解会优先提取低阶次，不适合高阶波纹度分析
2. 直接对目标阶次进行最小二乘拟合更准确
3. 预处理是关键：需要消除低频趋势
"""
...
def fit_all_orders_simultaneously(theta, signal, orders):
    """
    同时拟合所有目标阶次
    ...
    # 构建矩阵：每个阶次有sin和cos两列
    A_matrix = np.zeros((len(theta), 2 * n_orders))
    for i, order in enumerate(orders):
        A_matrix[:, 2*i] = np.sin(order * theta)
        A_matrix[:, 2*i + 1] = np.cos(order * theta)
    ...
    for i, order in enumerate(orders):
        A_coef = result[2*i]
        B_coef = result[2*i + 1]
        spectrum[order] = np.sqrt(A_coef**2 + B_coef**2)
    
    return spectrum


def analyze_final(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    print(f"\n{'='*80}")
    print(f"最终版波纹度分析: {sample_name}")
    ...
    for surface_name, surface_key, data_type in [
        ...
        # 同时拟合所有目标阶次
        spectrum = fit_all_orders_simultaneously(theta, dev_proc, target_orders)
