...
基于Klingelnberg文档和Gleason文档的综合分析
"""
...
def generate_final_report(mka_file, sample_name, poly_order=3):
    """生成最终分析报告"""
    ...
    surfaces = [
        ('INVOLUTE LEFT', 'left_profile', 'profile', axes[0, 0]),
        ('INVOLUTE RIGHT', 'right_profile', 'profile', axes[0, 1]),
        ('LEAD LEFT', 'left_helix', 'helix', axes[1, 0]),
        ('LEAD RIGHT', 'right_helix', 'helix', axes[1, 1])
    ]
    ...
    for surface_name, surface_key, data_type, ax in surfaces:
        ...
        for order, amp in zip(top_orders, top_amplitudes):
            color = 'ro' if order % ZE == 0 else 'go'
            ax.plot(order, amp, color, markersize=8)
    ...
    # 保存图像
    output_file = mka_file.replace('.mka', '_final_report.png')
...
def main():
    ...
    print("波纹度分析最终报告")
    print("="*80)
    print("\n基于Klingelnberg和Gleason文档的综合分析")
    print("\n关键算法：")
    print("1. 曲线合并：使用旋转角度法")
    print("2. 预处理：消除低频趋势（每齿单独处理）")
    print("3. 频谱分析：最小二乘正弦拟合")
    print("4. 识别：啮合谐波（ZE的倍数）和鬼阶次")
    
    # 分析样本1
    results1 = generate_final_report(
        "263751-018-WAV.mka",
        "样本1: 263751-018-WAV",
        poly_order=3
    )
    ...
    # 分析样本2
    results2 = generate_final_report(
        "004-xiaoxiao1.mka",
        "样本2: 004-xiaoxiao1",
        poly_order=3
    )
