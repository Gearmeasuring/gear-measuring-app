...
class MockInfo:
    ...
    def __init__(self):
        ...
        self.base_diameter = 87.0  # 假设基圆直径为87mm
...
class MockMeasurementData:
    ...
    def __init__(self):
        ...
        # 为每个齿生成模拟数据，更接近实际测量数据
        for tooth_id in range(1, 5):  # 生成4个齿的数据
            # 生成正弦波数据，包含多个阶次，更接近实际测量数据
            n_points = 100
            x = np.linspace(0, 2 * np.pi, n_points)
            
            # 生成包含多个阶次的信号，与参考图片一致
            signal = 0.15 * np.sin(87 * x)  # 87阶（齿数），幅值0.15
            signal += 0.07 * np.sin(174 * x)  # 174阶（2*87），幅值0.07
            signal += 0.06 * np.sin(261 * x)  # 261阶（3*87），幅值0.06
            signal += 0.05 * np.sin(348 * x)  # 348阶（4*87），幅值0.05
            signal += 0.04 * np.sin(435 * x)  # 435阶（5*87），幅值0.04
            signal += 0.03 * np.sin(522 * x)  # 522阶（6*87），幅值0.03
        ...
        for tooth_id in range(1, 5):  # 生成4个齿的数据
            ...
            # 生成包含多个阶次的信号
            signal = 0.15 * np.sin(87 * x)  # 87阶（齿数）
            signal += 0.07 * np.sin(174 * x)  # 174阶（2*87）
            signal += 0.06 * np.sin(261 * x)  # 261阶（3*87）
            signal += 0.05 * np.sin(348 * x)  # 348阶（4*87）
            signal += 0.04 * np.sin(435 * x)  # 435阶（5*87）
            signal += 0.03 * np.sin(522 * x)  # 522阶（6*87）
            signal += np.random.normal(0, 0.005, n_points)  # 添加噪声
