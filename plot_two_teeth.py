...
def plot_two_teeth():
    ...
    for tooth_num, angles in eval_teeth_angles.items():
        if angles:
            ...
            if angles:
                ...
                print(f"Tooth {tooth_num} angle range: {min_angle:.2f} to {max_angle:.2f} degrees")
    
    if all_angles:
        overall_min_angle = min(all_angles)
        overall_max_angle = max(all_angles)
        print(f"Overall angle range: {overall_min_angle:.2f} to {overall_max_angle:.2f} degrees")
    else:
        print("No angles available")
        return
    
    # 创建PDF报告
    output_pdf = 'two_teeth_profile_263751-018-WAV.pdf'
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        ax.set_title(f'{side.capitalize()} Profile - First Two Teeth', fontsize=14, fontweight='bold')
        ...
        # 绘制每个齿的数据
        for tooth_num in [1, 2]:
            if tooth_num in eval_teeth_data and tooth_num in eval_teeth_angles:
                ax.plot(eval_teeth_angles[tooth_num], eval_teeth_data[tooth_num], 
    ...
    print(f"Two teeth profile analysis completed. Results saved to: {output_pdf}")
