#!/usr/bin/env python3
"""
左齿向去鼓形、倾斜后的评价范围内的曲线，按照旋转角的公式合并，生成图表查看每一段曲线是否正常
"""
...
def process_and_merge_left_flank_evaluation_range():
    """
    处理左齿向评价范围内的数据，去鼓形和倾斜，然后按照旋转角公式合并
    """
    print("=== 处理左齿向评价范围内的数据，去鼓形和倾斜，按照旋转角公式合并 ===")
    ...
    print(f"左齿向数量: {len(left_flank_data)}")
    ...
    sorted_tooth_ids = sorted(left_flank_data.keys(), key=extract_number)
    ...
    for i, tooth_id in enumerate(sorted_tooth_ids):
        # 获取原始数据
        raw_data = left_flank_data[tooth_id]
...
def plot_merged_left_flank_evaluation_range():
    """
    生成合并后的左齿向评价范围内的曲线图表
    """
    # 处理和合并数据
    (
        ...
        rotation_angles_per_tooth,
        evaluation_ranges_per_tooth
    ) = process_and_merge_left_flank_evaluation_range()
    ...
    ax1.set_title(f'Merged Left Flank Curve (Evaluation Range) (z = {len(sorted_tooth_ids)})', fontsize=12, fontweight='bold')
    ...
    # 保存图表
    output_file = 'left_flank_evaluation_range_merged.png'
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"\n=== 图表生成完成 ===")
    print(f"合并后的左齿向评价范围曲线图表已保存为: {output_file}")
...
def plot_individual_teeth_evaluation_range():
    ...
    for i, tooth_id in enumerate(sorted_tooth_ids[:10]):
        ...
        if tooth_index is not None:
            ...
            # 添加标题和标签
            ax.set_title(f'Tooth {tooth_id} - Left Flank Evaluation Range (Indices: {start_idx}-{end_idx})', fontsize=12, fontweight='bold')
            ...
            # 保存图表
            output_file = f'tooth_{tooth_id}_left_flank_evaluation_correction.png'
            plt.savefig(output_file, dpi=150, bbox_inches='tight')
            print(f"生成齿 {tooth_id} 的左齿向评价范围图表: {output_file}")
    ...
    for tooth_id in specified_teeth:
        if tooth_id in left_flank_data:
            # 获取原始数据
            raw_data = left_flank_data[tooth_id]
            ...
            print(f"齿 {tooth_id} 不在左齿向数据中")


if __name__ == '__main__':
    plot_merged_left_flank_evaluation_range()
