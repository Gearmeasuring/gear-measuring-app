...
def involute_angle(r, rb):
    """计算渐开线展角"""
    alpha_r = np.arccos(np.clip(rb / r, -1, 1))
...
def prepare_profile_data(profile_dict, alpha_deg=20.0):
    """
    将解析的齿形数据转换为分析格式
    profile_dict: {tooth_num: [r1, r2, ..., rn, dev1, dev2, ..., devn]}
    """
    profiles = []
...
def prepare_helix_data(flank_dict):
    """
    将解析的齿向数据转换为分析格式
    flank_dict: {tooth_num: [z1, z2, ..., zn, dev1, dev2, ..., devn]}
    """
    flank_lines = []
...
def adaptive_preprocess(phi, dev, poly_order):
    """自适应预处理 - 每齿单独处理"""
    # 根据角度变化分段
    dphi = np.diff(phi)
...
def iterative_sine_decomposition(phi, dev, ZE, max_components=15, min_amplitude=0.002):
    """迭代正弦波分解"""
    residual = dev.copy()
    components = []
    ...
    # 找出峰值频率
    peak_indices = signal.find_peaks(positive_amps, height=np.max(positive_amps)*0.05)[0]
    peak_freqs = positive_freqs[peak_indices]
    peak_amps = positive_amps[peak_indices]
    
    sort_idx = np.argsort(peak_amps)[::-1]
    top_freqs = peak_freqs[sort_idx[:max_components*2]]
    ...
    for _ in range(max_components):
        ...
        for freq in top_freqs:
            ...
            guess_amp = np.std(residual) * np.sqrt(2)
            
            try:
                result = least_squares(sine_func, [guess_amp, 0], 
                                      bounds=([0, -np.pi], [np.inf, np.pi]))
                
                if result.cost < best_score:
                    best_score = result.cost
                    best_params = (freq, result.x[0], result.x[1])
            except:
                continue
        
        if best_params is None or best_params[1] < min_amplitude:
            break
        
        freq, amp, phase = best_params
...
def calculate_spectrum(components, ZE, max_order=None):
    ...
    for comp in components:
        order = int(round(comp['frequency'] * ZE))
        
        if 1 <= order <= max_order:
            if order in spectrum:
                spectrum[order] = max(spectrum[order], comp['amplitude'])
            else:
...
def analyze_with_adaptive(mka_file, sample_name, klingelnberg_ref):
    """使用自适应方法分析样本"""
    print(f"\n{'='*80}")
    print(f"自适应分析: {sample_name}")
    print(f"{'='*80}\n")
    ...
    print(f"\n自适应参数: 齿形多项式阶数={profile_poly}, 齿向多项式阶数={helix_poly}")
    ...
    # 分析四个面
    for surface_name, surface_key, data_type, poly_order in [
        ('Right Profile', 'right_profile', 'profile', profile_poly),
        ('Left Profile', 'left_profile', 'profile', profile_poly),
        ('Right Helix', 'right_helix', 'helix', helix_poly),
        ('Left Helix', 'left_helix', 'helix', helix_poly)
    ]:
        ...
        # 自适应预处理
        phi_proc, dev_proc = adaptive_preprocess(phi, dev, poly_order)
        
        # 迭代分解
        components = iterative_sine_decomposition(phi_proc, dev_proc, ZE, min_amplitude=min_amp)
