...
class FinalUnifiedRippleAnalyzer:
    """最终统一标准波纹度分析器"""
    ...
    def build_merged_curve(self, side_data, side, data_type, eval_start, eval_end, meas_start=0, meas_end=0):
        """构建合并曲线"""
        if not side_data:
            return None, None
        ...
        for tooth_id in sorted_teeth:
            ...
            if data_type == 'profile':
                ...
                if abs(self.helix_angle) > 0.01 and self.pitch_diameter > 0:
                    ...
                    delta_phi = np.zeros(eval_points)
...
def analyze_sample_final(mka_file, sample_name, klingelnberg_ref):
    """使用最终统一算法分析样本"""
    print(f"\n{'='*90}")
    print(f"Final Unified Analysis: {sample_name}")
    print(f"{'='*90}")
    ...
    analyzer = FinalUnifiedRippleAnalyzer(
    ...
    print(f"  Amplitude Scale = {analyzer.amplitude_scale}")
    ...
    for direction in ['right_profile', 'left_profile', 'right_helix', 'left_helix']:
        ...
        spectrum = analyzer.compute_spectrum(angles, values, target_orders)
