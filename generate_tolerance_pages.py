...
"""
    ...
    # 替换原有代码
    return re.sub(init_ui_pattern, f'\\1{new_pages_code}', content)
...
"""
    ...
    new_logic = """
        ...
        if page_id in page_mapping:
            ...
            self.content_stack.setCurrentIndex({len(standards) * 3})  # Empty page
            self.setWindowTitle("Tolerance Settings")
"""
    ...
    # 移除旧的映射和逻辑
    old_method = re.sub(r'        # Map IDs to stack indices[\s\S]*?        self.content_stack.setCurrentIndex\(3\)[\s\S]*?        self.setWindowTitle\("Tolerance Settings"\)', 
