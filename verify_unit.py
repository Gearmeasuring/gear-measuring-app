...
def verify_unit_conversion(mka_file, sample_name, klingelnberg_ref):
    """验证单位转换"""
    print(f"\n{'='*90}")
    print(f"Verify Unit Conversion: {sample_name}")
    print(f"{'='*90}")
    ...
    for side in ['right', 'left']:
        ...
        for scale in np.linspace(0.01, 0.5, 100):
            ...
            for order in ref.keys():
                our_amp = spectrum.get(order, 0) * scale
                ref_amp = ref[order]
        ...
        # 显示结果
        print(f"\n  {'Order':<8} {'Ours(um)':<12} {'Scaled':<12} {'Klingelnberg':<12} {'Error':<10}")
        for order in sorted(ref.keys()):
            our_amp = spectrum.get(order, 0)
            scaled_amp = our_amp * best_scale
            ref_amp = ref[order]
            error = abs(scaled_amp - ref_amp) / ref_amp * 100
...
def main():
    ...
    verify_unit_conversion(sample2_file, "Sample2 (ZE=26)", KLINGELNBERG_SAMPLE2)
