...
关键发现：
1. 齿向数据：使用展长到角度映射，结果接近Klingelnberg报告（1.16x）
2. 齿形数据：需要 ep × 4~5 的缩放因子

算法：
1. Profile: 展长 s(d) = sqrt((d/2)^2 - (db/2)^2) → 旋转角 ξ = s/(π×db)×360°
2. Helix: 轴向位置 z → 旋转角 α₂ = 2×Δz×tan(β₀)/D₀
3. 合并所有齿形成360°闭合曲线
4. 齿形数据缩放因子：ep × 4.5（左右齿形的平均值）
5. 迭代提取：每次提取幅值最大的阶次
6. 评价：只考虑阶次≥ZE的成分
"""
import os
...
def analyze_ripple(mka_file):
    """分析波纹度"""
    print("="*70)
    print("波纹度频谱分析最终版本 V26")
    print("="*70)
    print("\n关键发现:")
    print("1. 齿向数据：使用展长到角度映射，结果接近Klingelnberg报告")
    print("2. 齿形数据：需要 ep × 4.5 的缩放因子")
