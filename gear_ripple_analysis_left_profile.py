...
class LeftProfileAnalyzer:
    ...
    def calculate_average_curve(self, data_dict):
        ...
        for values in data_dict.values():
            ...
            # 提取评价范围（使用更精确的范围）
            # 对于480个数据点，评价范围通常是从第100个点到第400个点
            start_idx = 100
            end_idx = 400
            eval_curve = curve[start_idx:end_idx]
            
            if len(eval_curve) < 250:
                continue
        ...
        avg_curve = avg_curve[np.abs(avg_curve - mean) < 3 * std]
    ...
    def analyze_left_profile(self, file_path, teeth_count=87):
        """分析左齿形"""
        # 提取左齿形数据
...
if __name__ == "__main__":
    analyzer = LeftProfileAnalyzer()
    
    # 分析左齿形
    file_path = "263751-018-WAV.mka"
    
    print("分析左齿形...")
    left_profile_result = analyzer.analyze_left_profile(file_path, 87)
