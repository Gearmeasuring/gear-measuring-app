"""
波纹度频谱分析 V16 - 测试不同滤波器参数

关键发现：
- V14无滤波器：齿向数据非常接近（1.16x-1.17x）
- 齿形数据差距约5-7倍
- RC 100000:1太强，滤掉了所有高频成分

测试不同滤波器参数来匹配Klingelnberg报告
"""
import os
import sys
import math
...
def apply_low_pass_filter(data, cutoff_order, total_points):
    """
    基于阶次的低通滤波器
    
    cutoff_order: 截止阶次（如87阶）
    total_points: 数据点数
    """
...
def test_filter_params(mka_file):
    """测试不同滤波器参数"""
    print("="*70)
    print("波纹度频谱分析 V16 - 测试不同滤波器参数")
    print("="*70)
    ...
    filter_configs = [
        {'name': '无滤波器', 'cutoff': None},
        {'name': '截止87阶', 'cutoff': 87},
        {'name': '截止174阶', 'cutoff': 174},
        {'name': '截止261阶', 'cutoff': 261},
        {'name': '截止350阶', 'cutoff': 350},
    ]
    ...
    for config in filter_configs:
        ...
        for side, data_type, name, data_source, eval_start, eval_end in directions:
            ...
            if config['cutoff'] is not None:
                values = apply_low_pass_filter(values, config['cutoff'], len(values))
        ...
        for name, amp in results.items():
            ...
            print(f"{name:<10} {amp:<15.4f} {k_val:<15.2f} {ratio:<10.2f}x")
