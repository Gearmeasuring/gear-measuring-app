...
def create_contact_pattern_chart_v3(profile_data=None, pitch_data=None, gear_data=None,
                                    output_path=None, max_teeth=None):
    """
    创建考虑螺距的轮廓测量序列图（正确的展开方式）
    ...
    plot_unfolded_profile_v3(ax1, left_profile, left_pitch, z, tooth_angle,
                             '左齿面 - 考虑螺距的轮廓测量序列',
                             gear_data, max_teeth)
    ...
    plot_unfolded_profile_v3(ax2, right_profile, right_pitch, z, tooth_angle,
                             '右齿面 - 考虑螺距的轮廓测量序列',
                             gear_data, max_teeth)
...
def plot_unfolded_profile_v3(ax, profile_data, pitch_data, z, tooth_angle, title,
                             gear_data, max_teeth=None):
    """
    绘制展开式轮廓偏差曲线（V3 - 正确的理解）

    核心思想：
    - 将每个齿的轮廓数据（480个点）映射到对应的角度范围
    - 齿1：0° ~ tooth_angle
    - 齿2：tooth_angle ~ 2*tooth_angle
    - ...
    - 形成覆盖全周的连续曲线
    """
    ...
    for i, tooth_id in enumerate(tooth_ids):
        ...
        # ========== 正确的算法 ==========
        # 1. 计算该齿的起始角度（基于齿号）
        base_theta_start = (tooth_id - 1) * tooth_angle
        ...
        # 该齿的实际起始角度（考虑节距误差）
        theta_start = base_theta_start + pitch_adjustment

        # 3. 将该齿的n_points个点均匀分布在 tooth_angle 的角度范围内
        # 每个点对应的角度步长
        angle_step = tooth_angle / n_points

        # 生成该齿所有点的角度坐标
        theta_points = theta_start + np.arange(n_points) * angle_step
    ...
    # 设置坐标轴
    ax.set_xlabel('齿轮旋转角度 θ (°)', fontsize=11)
    ax.set_ylabel('偏差 (μm)', fontsize=11)
    ax.set_title(title, fontsize=12, fontweight='bold', pad=8)

    # 设置X轴范围
    if max_teeth:
        ax.set_xlim(-2, max_teeth * tooth_angle + 2)
    else:
        ax.set_xlim(-5, 360 + 5)
...
if __name__ == '__main__':
    ...
    # 测试1：前3齿放大视图
    print("生成前3齿放大视图...")
    fig1 = create_contact_pattern_chart_v3(
        ...
        output_path='contact_pattern_v3_zoomed_3teeth.png',
        max_teeth=3
    )

    # 测试2：全部10齿（显示在0-360°范围内）
    print("生成全部齿视图...")
    fig2 = create_contact_pattern_chart_v3(
        profile_data={'left': test_profile_left, 'right': test_profile_right},
        pitch_data={'left': test_pitch_left, 'right': test_pitch_right},
        gear_data=test_gear_data,
        output_path='contact_pattern_v3_all.png',
