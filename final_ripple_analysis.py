...
def iterative_sine_fitting_final(phi, dev, ZE, max_components=20, min_amplitude=0.002):
    """
    迭代拟合正弦分析 - 最终修正版
...
def analyze_with_final_method(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    print(f"\n{'='*80}")
    print(f"最终修正版波纹度分析: {sample_name}")
    ...
    for surface_name, surface_key, data_type in [
        ...
        components = iterative_sine_fitting_final(phi_proc, dev_proc, ZE)
...
def main():
    for poly_order, name in [(2, '消除鼓形 (2阶)'), (3, '消除鼓形和齿距 (3阶)')]:
        ...
        error1 = analyze_with_final_method(
            "263751-018-WAV.mka",
        ...
        error2 = analyze_with_final_method(
            "004-xiaoxiao1.mka",
