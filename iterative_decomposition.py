...
def iterative_sine_decomposition(phi, dev, ZE, max_components=30, min_amplitude=0.001):
    """
    迭代正弦分解
    
    每次迭代：
    1. 用FFT找出当前残差中的最大振幅频率
    2. 用最小二乘精确拟合该频率的正弦波
    3. 从残差中减去该正弦波
    4. 重复直到满足停止条件
    """
...
def analyze_with_iterative_decomposition(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    print(f"\n{'='*80}")
    print(f"迭代正弦分解分析: {sample_name}")
    ...
    for surface_name, surface_key, data_type in [
        ...
        # 迭代分解
        components = iterative_sine_decomposition(phi_proc, dev_proc, ZE)
...
def main():
    ...
    for poly_order in [2, 3, 4]:
        ...
        # 分析样本1
        error1 = analyze_with_iterative_decomposition(
            "263751-018-WAV.mka",
        ...
        # 分析样本2
        error2 = analyze_with_iterative_decomposition(
            "004-xiaoxiao1.mka",
