...
def iterative_decomposition_for_orders(phi, dev, ZE, target_orders, max_iterations=3):
    """
    对目标阶次进行迭代分解
    ...
    for iteration in range(max_iterations):
        for order in target_orders:
            amp, phase = fit_sine_at_order(phi, residual, order, ZE)
        ...
        for order in spectrum:
            freq = order / ZE
            sine_wave = spectrum[order] * np.sin(freq * phi + phases[order])
...
def analyze_final(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    ...
    for surface_name, surface_key, data_type in [
        ...
        # 对目标阶次进行迭代分解
        spectrum = iterative_decomposition_for_orders(phi_proc, dev_proc, ZE, target_orders)
...
def main():
    ...
    print("="*80)
    print("\n关键发现：")
    print("1. 阶次计算：阶次 = 频率 * ZE")
    print("2. 预处理是关键：需要消除低频趋势")
    print("3. 对目标阶次进行迭代分解比FFT更准确")
