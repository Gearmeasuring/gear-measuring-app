...
# -*- coding: utf-8 -*-
"""
基于拟合正弦的波纹度分析 - Klingelnberg标准方法

核心算法：
1. 首先用FFT识别主要频率成分
2. 对每个主要频率进行精确的最小二乘正弦拟合
3. 迭代提取，每次提取后从信号中减去已提取的成分
"""
...
def iterative_sine_fitting(phi, dev, ZE, max_components=15, min_amplitude=0.002):
    """
    迭代拟合正弦分析 - Klingelnberg标准方法
    ...
    for iteration in range(max_components):
        ...
        try:
            ...
            result = least_squares(
                sine_func,
                [guess_freq, guess_amp, guess_phase],
...
def analyze_with_sine_fitting(mka_file, sample_name, klingelnberg_ref, poly_order=3):
    """
    使用拟合正弦方法分析样本
    
    Args:
        poly_order: 预处理多项式阶数 (2=消除鼓形, 3=消除鼓形和齿距)
    """
    print(f"\n{'='*80}")
    print(f"拟合正弦方法分析: {sample_name}")
    ...
    for surface_name, surface_key, data_type in [
        ...
        # 迭代拟合正弦分析
        components = iterative_sine_fitting(phi_proc, dev_proc, ZE)
...
def main():
    ...
    for poly_order, name in [(2, '消除鼓形 (2阶)'), (3, '消除鼓形和齿距 (3阶)')]:
        ...
        # 分析样本1
        error1 = analyze_with_sine_fitting(
            "263751-018-WAV.mka",
        ...
        # 分析样本2
        error2 = analyze_with_sine_fitting(
            "004-xiaoxiao1.mka",
