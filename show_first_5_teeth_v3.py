"""
================================================================================
前五个齿的合并曲线放大显示（版本3 - 使用实际算法并选择中间z位置）
================================================================================
"""
...
print("="*80)
print("前五个齿的合并曲线放大显示（使用实际算法，只选中间z位置）")
print("="*80)
...
print()
print("[评价范围]")
print(f"  齿形评价范围: d1={profile_eval.eval_start}mm, d2={profile_eval.eval_end}mm")
print(f"  齿向评价范围: b1={helix_eval.eval_start}mm, b2={helix_eval.eval_end}mm")
print(f"  齿向中间点: {(helix_eval.eval_start + helix_eval.eval_end)/2}mm")

# 右齿形
print()
print("="*80)
print("[右齿形前5个齿 - 只选中间z位置]")
print("="*80)

profile_data_right = analyzer.reader.profile_data.get('right', {})

# 只取前5个齿，并且每个齿只选最接近齿向评价范围中间位置的z位置
helix_mid = (helix_eval.eval_start + helix_eval.eval_end) / 2

profile_data_right_filtered = {}
for tooth_id in sorted(profile_data_right.keys())[:5]:
    tooth_profiles = profile_data_right[tooth_id]
    # 找到最接近中间点的z位置
    best_z = min(tooth_profiles.keys(), key=lambda z: abs(z - helix_mid))
    profile_data_right_filtered[tooth_id] = {best_z: tooth_profiles[best_z]}
    print(f"  齿{tooth_id}: 选择 z={best_z}mm (最接近中间点{helix_mid}mm)")

angles_right, values_right = profile_calc.build_rotation_curve(
    profile_data_right_filtered, 
    profile_eval, 'right',
    meas_range=profile_eval,
    helix_eval_range=helix_eval
)

print(f"\n  总数据点数: {len(values_right)}")
print(f"  值范围: [{values_right.min():.4f}, {values_right.max():.4f}] um")

# 左齿形
print()
print("="*80)
print("[左齿形前5个齿 - 只选中间z位置]")
print("="*80)

profile_data_left = analyzer.reader.profile_data.get('left', {})

profile_data_left_filtered = {}
for tooth_id in sorted(profile_data_left.keys())[:5]:
    tooth_profiles = profile_data_left[tooth_id]
    # 找到最接近中间点的z位置
    best_z = min(tooth_profiles.keys(), key=lambda z: abs(z - helix_mid))
    profile_data_left_filtered[tooth_id] = {best_z: tooth_profiles[best_z]}
    print(f"  齿{tooth_id}: 选择 z={best_z}mm (最接近中间点{helix_mid}mm)")
...
ax1.set_title('Right Profile - First 5 Teeth (Middle z only)')
...
ax2.set_title('Left Profile - First 5 Teeth (Middle z only)')
...
plt.tight_layout()
plt.savefig(os.path.join(current_dir, 'first_5_teeth_v3.png'), dpi=150, bbox_inches='tight')
print()
print("图表已保存: first_5_teeth_v3.png")
