...
def sine_fitting_analysis(phi, dev, ZE, max_order=None, min_amplitude=0.002):
    """
    拟合正弦分析 - Klingelnberg标准方法
    
    迭代提取正弦波分量，每次提取最大振幅的分量
    """
    ...
    for iteration in range(20):
        ...
        # 在频率范围内搜索
        freq_range = np.linspace(min_freq, max_freq, 500)
...
def analyze_with_klingelnberg_method(mka_file, sample_name, klingelnberg_ref, evaluation_type='all'):
    ...
    for surface_name, surface_key, data_type in [
        ...
        # 拟合正弦分析
        components = sine_fitting_analysis(phi_proc, dev_proc, ZE)
