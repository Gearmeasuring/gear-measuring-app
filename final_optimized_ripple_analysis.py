...
关键发现：
1. 齿形数据：使用合并曲线方法，预处理阶数3-4效果最好
2. 齿向数据：使用每个齿单独计算频谱然后平均的方法，避免角度合成问题
"""
import os
import sys
import numpy as np
...
class FinalOptimizedRippleAnalyzer:
    """最终优化版波纹度分析器"""
    ...
    def compute_single_tooth_spectrum(self, angles, values, max_order=None):
        """计算单个齿的频谱"""
    ...
    def analyze_profile_merged(self, side_data, side, eval_start, eval_end, meas_start, meas_end, preprocess_order=3):
        """齿形分析 - 使用合并曲线方法"""
        if not side_data:
            return None
        ...
        for tooth_id in sorted_teeth:
            ...
            corrected_values = self.preprocess_tooth_data(eval_values, preprocess_order)
        ...
        return self.compute_single_tooth_spectrum(all_angles[sort_idx], all_values[sort_idx])
    
    def analyze_helix_per_tooth(self, side_data, side, eval_start, eval_end, preprocess_order=0):
        """齿向分析 - 使用每个齿单独计算频谱然后平均的方法"""
        if not side_data:
            return None
...
def analyze_sample_final(mka_file, sample_name, klingelnberg_ref):
    ...
    analyzer = FinalOptimizedRippleAnalyzer(
    ...
    print(f"  Delta_phi range = {analyzer.delta_phi_range:.2f}")
    print(f"  Profile: Merged curve method, preprocess order=3")
    print(f"  Helix: Per-tooth spectrum average, preprocess order=0 (mean removal only)")
    ...
    for direction in ['right_profile', 'left_profile', 'right_helix', 'left_helix']:
        ...
        if 'profile' in direction:
            ...
            spectrum = analyzer.analyze_profile_merged(
                side_data, side,
            ...
            spectrum = analyzer.analyze_helix_per_tooth(
                side_data, side,
        ...
        results[direction] = {'spectrum': spectrum, 'errors': errors, 'avg_error': np.mean(errors)}
...
def main():
    ...
    print(f"\nKey Improvements:")
    print(f"  1. Profile: Use merged curve method with order-3 polynomial preprocessing")
    print(f"  2. Helix: Use per-tooth spectrum averaging (avoids angle synthesis issues)")
    print(f"  3. Helix preprocessing: Mean removal only (no polynomial fitting)")
